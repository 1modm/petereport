...
from django.forms import Textarea, TextInput, DateInput, ModelChoiceField, CheckboxSelectMultiple, EmailField, BooleanField, FileInput, ModelMultipleChoiceField, URLInput
...
class NewCustomerForm(forms.ModelForm):
    ...
    class Meta:
        model = DB_Customer
        fields = ('name', 'contact_list', 'contact_sp_mail',
                  'contact_dp_mail', 'description', 'tags')
...
class NewProductForm(forms.ModelForm):
    ...
    class Meta:
        model = DB_Product
        fields = ('name', 'description', 'recovery', 'installation', 'usage', 'customer', 'tags')
...
class NewSettingsForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('company_name', 'company_website',
                  'company_address', 'company_picture')
...
class NewShareForm(forms.ModelForm):
    ...
    type = forms.ChoiceField(choices=type_choice, required=True, widget=forms.Select(attrs={
                             'class': 'form-control', 'id': 'selecttype', 'onchange': 'TypeChange(this.value);', 'type': "text", 'required': "required", 'placeholder': _("Deliverable/Findings")}))
    func_choice = ((x, _(x))
                   for l in [['none'], shares_finding, shares_deliverable] for x in l)
    func = forms.ChoiceField(choices=func_choice, required=True, widget=forms.Select(
        attrs={'class': 'form-control', 'id': 'selectfunc', 'type': "text", 'required': "required"}))
...
class NewFindingForm(forms.ModelForm):
    ...
    status = forms.ChoiceField(choices=status_choices, required=True, widget=forms.Select(
        attrs={'class': 'form-control', 'type': "text", 'required': "required", 'placeholder': _("Opened/Closed")}))
    ...
    owasp = OWASPModelChoiceField(queryset=DB_OWASP.objects.all(), empty_label=_(
        "(Select an OWASP ID)"), widget=forms.Select(attrs={'class': 'form-control select2OWASP'}))

    class Meta:
        model = DB_Finding
        fields = ('title', 'status', 'severity', 'cvss_score', 'cvss_base_score', 'description',
                  'poc', 'location', 'impact', 'recommendation', 'ref', 'cwe', 'owasp', 'tags')
...
class NewCSPNEvaluationForm(forms.ModelForm):
    ...
    status = forms.ChoiceField(choices=status_choices, required=True, widget=forms.Select(
        attrs={'class': 'form-control', 'type': "text", 'required': "required", 'placeholder': _("Evaluated/Not Evaluated")}))
    stage = CSPNEvaluationStageModelChoiceField(queryset=DB_CSPN_Evaluation_Stage.objects.all(), empty_label=_(
        "(Select a CSPN Stage ID)"), widget=forms.Select(attrs={'class': 'form-control select2CSPNEvaluationStage'}))
...
class NewFindingTemplateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('title', 'severity', 'cvss_score', 'cvss_base_score', 'description',
                  'location', 'impact', 'recommendation', 'ref', 'cwe', 'owasp', 'tags')
...
class NewAppendixForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["finding"] = FindingModelChoiceField(queryset=DB_finding_query, empty_label=_(
            "(Select a finding)"), widget=forms.Select(attrs={'class': 'form-control'}))
...
class AddUserForm(UserCreationForm):
    ...
    group = ModelChoiceField(queryset=Group.objects.all(), empty_label=_(
        "(Select a group)"), widget=forms.Select(attrs={'class': 'form-control'}))
    ...
    class Meta:
        model = User
        fields = ('username', 'group', 'superadmin',
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['username'].widget.attrs.update(
            {'class': 'form-control', 'type': "text", 'required': "required", 'placeholder': _("Username")})
...
class NewAttackTreeForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["finding"] = FindingModelChoiceField(
            queryset=DB_finding_query, empty_label="(Select a finding)", widget=forms.Select(attrs={'class': 'form-control'}))
...
class NewOWASPForm(forms.ModelForm):
    ...
    class Meta:
        model = DB_OWASP
        fields = ('owasp_id', 'owasp_year', 'owasp_name',
                  'owasp_description', 'owasp_url')
